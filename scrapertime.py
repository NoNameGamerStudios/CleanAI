...
def scrape_content(url):
    try:
        ...
        # Remove unwanted sections (navigation bars, footers, etc.)
        for tag in soup(['nav', 'footer', 'header', 'aside', 'form', 'button', 'script', 'style']):
            tag.decompose()

        # Also remove divs with common bad classes (menus, ads)
        bad_keywords = ['menu', 'nav', 'footer', 'header', 'aside', 'advert', 'ads', 'sidebar', 'popup', 'cookie']
        for div in soup.find_all('div'):
            classes = div.get('class') or []
            if any(bad_word in ' '.join(classes).lower() for bad_word in bad_keywords):
                div.decompose()
        ...
        if not main_content:
            candidates = ['main-content', 'article', 'post', 'content', 'entry-content']
